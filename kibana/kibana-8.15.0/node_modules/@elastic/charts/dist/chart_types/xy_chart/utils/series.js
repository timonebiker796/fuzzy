...
function splitSeriesDataByAccessors(spec, xValueSums, isStacked = false, isBanded = false, stackMode, groupBySpec) {
    var _a, _b, _c, _d, _e;
    ...
    const xValues = [];
    const nonNumericValues = new Map();
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        yAccessors.forEach((accessor, index) => {
            var _a;
            const cleanedDatum = extractYAndMarkFromDatum(datum, accessor, nonNumericValues, isBanded, y0Accessors && y0Accessors[index], markSizeAccessor);
    ...
    if (nonNumericValues.size > 0) {
        ...
        logger_1.Logger.warn(`Found ${total} non-numeric y value${total > 1 ? 's' : ''} in dataset for spec "${specId}"`, `(${values.map(([k, v]) => `${v}: ${JSON.stringify(k)}`).join(', ')})`);
    }
    return {
        dataSeries,
        xValues,
...
function finiteOrNull(value, nonNumericValues) {
    ...
    if (!finite) {
        const val = (_a = nonNumericValues.get(value)) !== null && _a !== void 0 ? _a : 0;
...
const getSortedDataSeries = (dataSeries, xValues, xScaleType) => dataSeries.map(({ data, ...rest }) => ({
    ...rest,
    data: data.toSorted((0, stacked_series_utils_1.datumXSortPredicate)(xScaleType, [...xValues.values()])),
}));
...
function getDataSeriesFromSpecs(seriesSpecs, deselectedDataSeries = [], orderOrdinalBinsBy, groupBySpec) {
    ...
    for (const spec of seriesSpecs) {
        ...
        const { dataSeries, xValues } = splitSeriesDataByAccessors(spec, mutatedXValueSums, isStacked, isBanded, specGroup === null || specGroup === void 0 ? void 0 : specGroup.stackMode, groupBySpec);
...
function isBandedSpec(spec) {
    return Boolean(spec.y0Accessors && spec.y0Accessors.length > 0 && !(0, y_domain_1.isStackedSpec)(spec));
}
