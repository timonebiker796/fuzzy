...
function renderArea(shift, dataSeries, xScale, yScale, panel, color, curve, isBandedSpec, xScaleOffset, style, markSizeOptions, isStacked, hasFit, pointStyleAccessor) {
    ...
    if (y0Line)
        lines.push(y0Line);
    const { pointGeometries, indexedGeometryMap } = (0, points_1.renderPoints)(shift - xScaleOffset, dataSeries, xScale, yScale, panel, color, style.point, style.isolatedPoint, isBandedSpec, markSizeOptions, false, style.isolatedPoint.enabled, pointStyleAccessor);
    const areaGeometry = {
