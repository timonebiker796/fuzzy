...
class SearchService {
  ...
  performFind(params, options, request) {
    ...
    const timeout$ = (0, _rxjs.timer)(this.config.search_timeout.asMilliseconds()).pipe((0, _rxjs.map)(mapToUndefined));
    ...
    const providersResults$ = [...this.providers.values()].map(provider => provider.find(params, findOptions, context).pipe((0, _rxjs.catchError)(() => _rxjs.EMPTY), (0, _operators.takeInArray)(this.maxProviderResults), (0, _rxjs.takeUntil)(aborted$), (0, _rxjs.map)(results => results.map(r => processResult(r)))));
    return (0, _rxjs.merge)(...providersResults$).pipe((0, _rxjs.map)(results => ({
      results
    })));
  }
}
