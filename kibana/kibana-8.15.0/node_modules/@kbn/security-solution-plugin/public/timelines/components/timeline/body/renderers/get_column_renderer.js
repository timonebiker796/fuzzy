"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.getColumnRenderer = void 0;
/*
 * Copyright Elasticsearch B.V. and/or licensed to Elasticsearch B.V. under one
 * or more contributor license agreements. Licensed under the Elastic License
 * 2.0; you may not use this file except in compliance with the Elastic License
 * 2.0.
 */

const unhandledColumnRenderer = () => {
  throw new Error('Unhandled Column Renderer');
};
const getColumnRenderer = (columnName, columnRenderers, data, context) => {
  const renderer = columnRenderers.find(columnRenderer => columnRenderer.isInstance(columnName, data, context));
  return renderer != null ? renderer : unhandledColumnRenderer();
};
exports.getColumnRenderer = getColumnRenderer;