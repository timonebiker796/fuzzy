...
const profileUidColumns = exports.profileUidColumns = ['kibana.alert.workflow_assignee_ids', 'kibana.alert.workflow_user'];
const useFetchPageContext = ({
  alerts,
  columns
}) => {
  const uids = (0, _react.useMemo)(() => {
    const ids = new Set();
    alerts.forEach(alert => {
      profileUidColumns.forEach(columnId => {
        if (columns.find(column => column.id === columnId) != null) {
          const userUids = alert[columnId];
          userUids === null || userUids === void 0 ? void 0 : userUids.forEach(uid => ids.add(uid));
        }
      });
    });
    return ids;
