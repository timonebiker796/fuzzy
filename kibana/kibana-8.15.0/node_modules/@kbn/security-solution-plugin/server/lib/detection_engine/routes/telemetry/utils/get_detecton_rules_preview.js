...
const getDetectionRulesPreview = async ({
  ...
  await task.runTask('detection-rules-preview', logger, telemetryReceiver, taskSender, taskMetricsService, taskExecutionPeriod);
  const messages = taskSender.getSentMessages();
  const taskMetrics = taskMetricsService.getSentMessages();
  return (0, _parse_ndjson.parseNdjson)([...messages, ...taskMetrics]);
};
