...
function isEndpointProtectionUpdatesValidForLicense(policy, license) {
  if ((0, _license.isAtLeast)(license, 'enterprise')) {
    return true;
  }
  const defaults = (0, _policy_config.policyFactoryWithoutPaidEnterpriseFeatures)();
...
const isEndpointPolicyValidForLicense = (policy, license) => {
  return isEndpointMalwarePolicyValidForLicense(policy, license) && isEndpointRansomwarePolicyValidForLicense(policy, license) && isEndpointMemoryPolicyValidForLicense(policy, license) && isEndpointBehaviorPolicyValidForLicense(policy, license) && isEndpointAdvancedPolicyValidForLicense(policy, license) && isEndpointCredentialDumpingPolicyValidForLicense(policy, license) && isEndpointProtectionUpdatesValidForLicense(policy, license);
};
...
const unsetPolicyFeaturesAccordingToLicenseLevel = (policy, license) => {
  ...
  if ((0, _license.isAtLeast)(license, 'platinum')) {
    const policyWithSupportedFeatures = (0, _policy_config.policyFactoryWithSupportedFeatures)(policy);
    return (0, _policy_config.policyFactoryWithoutPaidEnterpriseFeatures)(policyWithSupportedFeatures);
  }
