...
class SentinelOneDataGenerator extends _endpoint_action_generator.EndpointActionGenerator {
  ...
  generateActivityFetchFileResponseData(overrides = {}) {
    const data = {
      flattened: {
      ...
      downloaded: {
        url: `/agents/${[...this.randomNGenerator(100, 4)].join('')}/uploads/${[...this.randomNGenerator(100, 4)].join('')}`
      },
      account: {
        name: 'Foo'
      }
    };
    return (0, _lodash.merge)(data, overrides);
  }
  generateSentinelOneApiActivityResponse(activityRecordOverrides = {}) {
    var _activityRecordOverri;
    const today = new Date();
    today.setMinutes(today.getMinutes() - 5);
    const activityType = (_activityRecordOverri = activityRecordOverrides.activityType) !== null && _activityRecordOverri !== void 0 ? _activityRecordOverri : this.randomActivityType();
    const activity = {
    ...
    return {
      data: [(0, _lodash.merge)(activity, activityRecordOverrides)],
      pagination: {
  ...
  generateActivityDataForType(activityType, overrides = {}) {
    let activityData = {
      accountName: 'elastic',
      groupName: 'Default Group'
    };
  ...
  generateSentinelOneApiAgentsResponse(agentDetailsOverrides = {}) {
    ...
    return {
      ...
      data: [(0, _lodash.merge)(agent, agentDetailsOverrides)],
      errors: null
    };
  }
}
...
const sentinelOneActivityTypes = {
