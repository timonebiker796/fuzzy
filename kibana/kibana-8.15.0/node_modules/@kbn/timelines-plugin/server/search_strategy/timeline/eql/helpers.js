...
const parseEqlResponse = async (options, response) => {
  ...
  if (response.rawResponse.hits.sequences !== undefined) {
    ...
    edges = await Promise.all(response.rawResponse.hits.events.map(async event => (0, _format_timeline_data.formatTimelineData)(options.fieldRequested, _constants2.TIMELINE_EVENTS_FIELDS, event)));
  }
  const inspect = {
