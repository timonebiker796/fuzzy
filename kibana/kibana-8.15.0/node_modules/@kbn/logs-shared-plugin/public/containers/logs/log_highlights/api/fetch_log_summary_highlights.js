...
const fetchLogSummaryHighlights = async (requestArgs, fetch) => {
  const response = await fetch(_http_api.LOG_ENTRIES_SUMMARY_HIGHLIGHTS_PATH, {
    method: 'POST',
    body: JSON.stringify(_http_api.logEntriesSummaryHighlightsRequestRT.encode(requestArgs)),
    version: '1'
  });
  return (0, _runtime_types.decodeOrThrow)(_http_api.logEntriesSummaryHighlightsResponseRT)(response);
};
