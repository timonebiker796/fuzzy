...
var _logs = require("./common/utils/logs");
...
async function migrationStateActionMachine({
  ...
  try {
    const finalState = await (0, _state_action_machine.stateActionMachine)(initialState, state => next(state), (state, res) => {
      ...
      lastState = state;
      (0, _logs.logActionResponse)(logger, logMessagePrefix, state, res);
      ...
      const now = Date.now();
      (0, _logs.logStateTransition)(logger, logMessagePrefix, state, redactedNewState, now - prevTimestamp);
      prevTimestamp = now;
      return newState;
    });
    ...
    if (finalState.controlState === 'DONE') {
      ...
      const errorMessage = `Unable to complete saved object migrations for the [${initialState.indexPrefix}] index: ` + finalState.reason;
      if (finalState.throwDelayMillis) {
        return new Promise((_, reject) => setTimeout(() => reject(errorMessage), finalState.throwDelayMillis));
      }
