...
function _default(directory) {
  ...
  const files = _lodash.default.map(_globby.default.sync('../' + directory + '/*.js', {
    cwd: __dirname
  }).filter(filename => !filename.includes('.test')), function (file) {
  ...
  const directories = _lodash.default.chain(_globby.default.sync('../' + directory + '/*/index.js', {
    cwd: __dirname
  })).map(function (file) {
