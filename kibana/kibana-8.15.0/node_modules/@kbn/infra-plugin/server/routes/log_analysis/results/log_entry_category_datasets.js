...
const initGetLogEntryCategoryDatasetsRoute = ({
  ...
  framework.registerVersionedRoute({
    access: 'internal',
    method: 'post',
    path: _http_api.logAnalysisResultsV1.LOG_ANALYSIS_GET_LOG_ENTRY_CATEGORY_DATASETS_PATH
  }).addVersion({
    version: '1',
    validate: {
      request: {
        body: (0, _runtime_types.createValidationFunction)(_http_api.logAnalysisResultsV1.getLogEntryCategoryDatasetsRequestPayloadRT)
      }
    }
    ...
    try {
      ...
      const {
        ...
        timing
      } = await (0, _log_analysis.getLogEntryCategoryDatasets)({
        infra: await infraMlContext.infra
      }, logView, idFormat, startTime, endTime);
      return response.ok({
        body: _http_api.logAnalysisResultsV1.getLogEntryCategoryDatasetsSuccessReponsePayloadRT.encode({
          data: {
