...
const initGetLogEntryAnomaliesRoute = ({
  ...
  framework.registerVersionedRoute({
    access: 'internal',
    method: 'post',
    path: _http_api.logAnalysisResultsV1.LOG_ANALYSIS_GET_LOG_ENTRY_ANOMALIES_PATH
  }).addVersion({
    version: '1',
    validate: {
      request: {
        body: (0, _runtime_types.createValidationFunction)(_http_api.logAnalysisResultsV1.getLogEntryAnomaliesRequestPayloadRT)
      }
    }
    ...
    try {
      ...
      const {
        ...
        hasMoreEntries,
        timing
      } = await (0, _log_analysis.getLogEntryAnomalies)(infraMlContext, logView, idFormats, startTime, endTime, sort, pagination, datasets);
      return response.ok({
        body: _http_api.logAnalysisResultsV1.getLogEntryAnomaliesSuccessReponsePayloadRT.encode({
          data: {
