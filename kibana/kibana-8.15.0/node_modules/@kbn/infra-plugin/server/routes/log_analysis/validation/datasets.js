...
const initValidateLogAnalysisDatasetsRoute = ({
  ...
  framework.registerVersionedRoute({
    access: 'internal',
    method: 'post',
    path: _http_api.logAnalysisValidationV1.LOG_ANALYSIS_VALIDATE_DATASETS_PATH
  }).addVersion({
    version: '1',
    validate: {
      request: {
        body: (0, _runtime_types.createValidationFunction)(_http_api.logAnalysisValidationV1.validateLogEntryDatasetsRequestPayloadRT)
      }
    }
    ...
    try {
      ...
      return response.ok({
        body: _http_api.logAnalysisValidationV1.validateLogEntryDatasetsResponsePayloadRT.encode({
          data: {
            datasets
          }
        })
