...
function createBuffer(bulkOperation, {
  ...
  storeUpdateBuffer.pipe((0, _rxjs.bufferWhen)(() => flushBuffer), (0, _rxjs.filter)(tasks => tasks.length > 0)).subscribe(bufferedEntities => {
  ...
  (0, _rxjs.flatMap)(() => {
    ...
    storeUpdateBuffer.pipe((0, _rxjs.bufferCount)(bufferMaxOperations - 1)),
    // flush buffer once max duration has passed
    (0, _rxjs.from)(resolveIn(bufferMaxDuration))]).pipe((0, _rxjs.first)(), (0, _rxjs.mapTo)(FLUSH)) : (0, _rxjs.from)([DONT_FLUSH]);
  }), (0, _rxjs.filter)(shouldFlush => shouldFlush)).subscribe({
