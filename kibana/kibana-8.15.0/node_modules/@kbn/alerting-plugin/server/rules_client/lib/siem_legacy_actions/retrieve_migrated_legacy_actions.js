...
exports.retrieveMigratedLegacyActions = void 0;
var _saved_objects = require("../../../saved_objects");
var _find_rules = require("../../../application/rule/methods/find/find_rules");
var _delete_rule = require("../../../application/rule/methods/delete/delete_rule");
 ...
 * @returns
 */
const retrieveMigratedLegacyActions = async (context, {
  ruleId
  ...
  try {
    ...
    const [siemNotification, legacyRuleActionsSO] = await Promise.all([(0, _find_rules.findRules)(context, {
      options: {
    ...
    const deleteLegacyActions = async () => {
      ...
      siemNotificationsExist && (0, _delete_rule.deleteRule)(context, {
        id: siemNotification.data[0].id
      }),
      // Delete the legacy sidecar SO if it exists
      legacyRuleNotificationSOsExist && unsecuredSavedObjectsClient.delete(_types.legacyRuleActionsSavedObjectType, legacyRuleActionsSO.saved_objects[0].id)]);
    };
    ...
    if (legacyRuleNotificationSOsExist) {
      ...
      const legacyActions = (0, _transform_legacy_actions.transformFromLegacyActions)(legacyRuleActionsSO.saved_objects[0].attributes, legacyRuleActionsSO.saved_objects[0].references);
      // only action references need to be saved
      const legacyActionsReferences = (_legacyRuleActionsSO$ = legacyRuleActionsSO.saved_objects[0].references.filter(({
        type
      }) => type === 'action')) !== null && _legacyRuleActionsSO$ !== void 0 ? _legacyRuleActionsSO$ : [];
...
exports.retrieveMigratedLegacyActions = retrieveMigratedLegacyActions;