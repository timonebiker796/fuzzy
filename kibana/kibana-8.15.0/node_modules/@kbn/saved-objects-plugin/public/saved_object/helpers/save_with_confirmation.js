 ...
 */
async function saveWithConfirmation(source, savedObject, options, services, startServices) {
  const {
    savedObjectsClient,
    overlays
  } = services;
  try {
    ...
    if ((0, _lodash.get)(err, 'res.status') === 409) {
      ...
      return (0, _confirm_modal_promise.confirmModalPromise)(confirmMessage, title, confirmButtonText, overlays, startServices).then(() => savedObjectsClient.create(savedObject.getEsType(), source, {
        overwrite: true,
        ...options
