...
function resolveCopySavedObjectsToSpacesConflictsFactory(savedObjects, request) {
  ...
  const resolveConflictsForSpace = async (spaceId, objectsStream, retries, createNewCopies, compatibilityMode) => {
    try {
  ...
  const resolveCopySavedObjectsToSpacesConflicts = async (sourceSpaceId, options) => {
    ...
    for (const entry of Object.entries(options.retries)) {
      ...
      response[spaceId] = await resolveConflictsForSpace(spaceId, (0, _readable_stream_from_array.createReadableStreamFromArray)(filteredObjects), retries, options.createNewCopies, options.compatibilityMode);
    }
    return response;
  };
