...
exports.RelativesFooter = void 0;
...
const RelativesFooter = props => {
  const {
    savedObjectTarget,
    referenceGraph,
    isDisabled
  } = props;
  const relativesCount = (0, _react.useMemo)(() => {
    const {
      type,
      id
    } = savedObjectTarget;
    return referenceGraph.filter(x => (x.type !== type || x.id !== id) && x.spaces.length > 0 && !x.isMissing).length;
...
exports.RelativesFooter = RelativesFooter;