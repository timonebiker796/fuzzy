...
class TimeBuckets {
  ...
  getInterval(useNormalizedEsInterval = true) {
    const duration = this.getDuration();
    const parsedInterval = isDurationInterval(this._i) ? this._i : (0, _calc_auto_interval.calcAutoIntervalNear)(this._timeBucketConfig['histogram:barTarget'], Number(duration));
    ...
    const decorateInterval = interval => {
      ...
      const durationDescription = (0, _calc_es_interval.getPreciseDurationDescription)(esInterval.value, prettyUnits);
    ...
    return decorateInterval(useNormalizedEsInterval ? maybeScaleInterval(parsedInterval) : parsedInterval);
  }
