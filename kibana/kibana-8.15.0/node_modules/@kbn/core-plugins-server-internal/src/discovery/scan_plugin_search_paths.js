...
function scanPluginSearchPaths(pluginDirs, log) {
  function recursiveScanFolder(ent) {
    return (0, _rxjs.from)([ent]).pipe((0, _rxjs.mergeMap)(entry => {
  ...
  return (0, _rxjs.from)(pluginDirs.map(dir => ({
    dir,
    depth: 0
  }))).pipe((0, _rxjs.mergeMap)(entry => {
    log.debug(`Scanning "${entry.dir}" for plugin sub-directories...`);
    return fsReadDir$(entry.dir).pipe((0, _rxjs.mergeMap)(() => recursiveScanFolder(entry)), (0, _rxjs.catchError)(err => [_plugin_discovery_error.PluginDiscoveryError.invalidSearchPath(entry.dir, err)]));
  }));
}
...
function findManifestInFolder(dir, notFound) {
  return fsStat$((0, _path.resolve)(dir, 'kibana.json')).pipe((0, _rxjs.mergeMap)(stats => {
    // `kibana.json` exists in given directory, we got a plugin
    if (stats.isFile()) {
      return [dir];
    }
    return [];
  }), (0, _rxjs.catchError)(manifestStatError => {
...
function mapSubdirectories(dir, mapFunc) {
  return fsReadDir$(dir).pipe((0, _rxjs.mergeMap)(subDirs => subDirs.map(subDir => (0, _path.resolve)(dir, subDir))), (0, _rxjs.mergeMap)(subDir => fsStat$(subDir).pipe((0, _rxjs.mergeMap)(pathStat => pathStat.isDirectory() ? mapFunc(subDir) : []), (0, _rxjs.catchError)(subDirStatError => [_plugin_discovery_error.PluginDiscoveryError.invalidPluginPath(subDir, subDirStatError)]))));
}