...
async function doesLogsIndexExist(req, logsIndexPattern, {
  start,
  end,
  clusterUuid,
  nodeUuid,
  ...
  const typeExistsAtAnyTimeQuery = {
    query: {
      bool: {
        filter: [_logs_filter.elasticsearchLogsFilter]
      }
    }
  };
  const typeExistsQuery = {
    query: {
      bool: {
        filter: [...filter, _logs_filter.elasticsearchLogsFilter]
      }
    }
  };
  const usingStructuredLogsQuery = {
    query: {
      bool: {
        filter: [...filter, _logs_filter.elasticsearchLogsFilter, structuredLogsFilter]
      }
    }
  };
  const clusterExistsQuery = {
    query: {
      bool: {
        filter: [...filter, _logs_filter.elasticsearchLogsFilter, clusterFilter]
      }
    }
  };
  const nodeExistsQuery = {
    query: {
      bool: {
        filter: [...filter, _logs_filter.elasticsearchLogsFilter, clusterFilter, nodeFilter]
      }
    }
  };
  const indexExistsQuery = {
    query: {
      bool: {
        filter: [...filter, _logs_filter.elasticsearchLogsFilter, clusterFilter, indexFilter]
      }
    }
  };
  const defaultParams = {
...
async function detectReason(req, logsIndexPattern, opts) {
