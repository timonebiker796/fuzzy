...
function getKibanasForClusters(req, clusters, ccs) {
  ...
  return Promise.all(clusters.map(cluster => {
    ...
    return callWithRequest(req, 'search', params).then(result => {
      ...
      const statusBuckets = (_aggregations$status$ = (_aggregations$status = aggregations.status) === null || _aggregations$status === void 0 ? void 0 : _aggregations$status.buckets) !== null && _aggregations$status$ !== void 0 ? _aggregations$status$ : [];
      ...
      if (kibanaUuids.length) {
        var _aggregations$request, _aggregations$concurr, _aggregations$respons, _aggregations$memory_, _aggregations$memory_2;
...
function hasStaleStatus(kibana) {
  var _kibana$latest_report, _kibana$latest_report2, _buckets$0$last_seen$, _buckets$, _buckets$$last_seen;
  const buckets = (_kibana$latest_report = kibana === null || kibana === void 0 ? void 0 : (_kibana$latest_report2 = kibana.latest_report) === null || _kibana$latest_report2 === void 0 ? void 0 : _kibana$latest_report2.buckets) !== null && _kibana$latest_report !== void 0 ? _kibana$latest_report : [];
  if (buckets.length === 0) {
    return true;
  }
  const lastSeenTimestamp = (_buckets$0$last_seen$ = (_buckets$ = buckets[0]) === null || _buckets$ === void 0 ? void 0 : (_buckets$$last_seen = _buckets$.last_seen) === null || _buckets$$last_seen === void 0 ? void 0 : _buckets$$last_seen.value_as_string) !== null && _buckets$0$last_seen$ !== void 0 ? _buckets$0$last_seen$ : null;
  if (lastSeenTimestamp === null) {
    return true;
  }
  return (0, _is_kibana_status_stale.isKibanaStatusStale)(lastSeenTimestamp);
}