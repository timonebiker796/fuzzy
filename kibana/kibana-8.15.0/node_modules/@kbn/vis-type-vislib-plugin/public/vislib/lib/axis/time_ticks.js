...
const timeTicks = scale => {
  ...
  const startOffset = (0, _moment.default)(domain[0]).utcOffset();
  const shiftedDomain = domain.map(val => (0, _moment.default)(val).add(startOffset, 'minute'));
  ...
  return n => {
    ...
    return ticks.map(d => {
      ...
      const currentOffset = (0, _moment.default)(d).utcOffset();
      return (0, _moment.default)(d).subtract(hourTicks ? startOffset : currentOffset, 'minute').valueOf();
    });
  };
};
