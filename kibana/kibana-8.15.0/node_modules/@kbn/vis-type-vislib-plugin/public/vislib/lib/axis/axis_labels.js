...
class AxisLabels {
    ...
    return function (selection) {
      ...
      if (config.get('labels.rotate')) {
        text.style('text-anchor', function () {
          const currentValue = (0, _jquery.default)(this).css('text-anchor');
          ...
          if ((0, _jquery.default)(this).css('text-anchor') === 'middle') {
  ...
  filterAxisLabels() {
    ...
    return function (selection) {
      ...
      const el = (0, _jquery.default)(config.get('rootEl')).find(config.get('elSelector'));
      ...
      selection.selectAll('.tick text').text(function (d) {
        const parentNode = _d.default.select(this.parentNode).node();
        const currentTickCenter = config.isHorizontal() ? scaleStartPad + self.axisScale.scale(d) : upperBound - scaleStartPad - self.axisScale.scale(d);
        ...
        if (outsideUpperBound || outsideLowerBound || overlapsLastTick) {
          _d.default.select(this.parentNode).remove();
        } else {
    ...
    return function (selection) {
      selection.each(function () {
        selection.selectAll('text').attr('style', function () {
          const currentStyle = _d.default.select(this).attr('style');
