...
class Layout {
  constructor(config) {
    ...
    this.opts = config;
    this.layoutType = _layout_types.layoutTypes[config.get('type')](this.el, this.data);
  }
   ...
   */
  createLayout(arr) {
    return _lodash.default.each(arr, obj => {
      this.layout(obj);
    });
  }
  updateCategoryAxisSize() {
    ...
    const axis = new _axis.Axis(visConfig, axisConfig);
    const position = axis.axisConfig.get('position');
    const chartTitle = new _chart_title.ChartTitle(visConfig);
    const axisWrapperElement = (0, _jquery.default)(this.el).find(`.visAxis__column--${position}`);
    ...
    axis.destroy();
    (0, _jquery.default)(this.el).find('.chart-title svg').remove();
    axisWrapperElement.css('visibility', '');
    if (axis.axisConfig.isHorizontal()) {
      const spacerNodes = (0, _jquery.default)(this.el).find(`.visAxis__spacer--y-${position}`);
  ...
  layout(obj) {
    ...
    if (obj.children) {
      ...
      _lodash.default.forEach(obj.children, function (obj) {
        if (!obj.parent) {
  ...
  appendElem(el, type, className) {
    ...
    if (typeof el === 'string') {
      ...
      el = _d.default.select(this.el).select(el)[0][0];
    }
    return _d.default.select(el).append(type).attr('class', className);
  }
