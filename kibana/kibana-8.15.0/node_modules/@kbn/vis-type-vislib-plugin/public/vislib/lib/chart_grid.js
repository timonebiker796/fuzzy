...
class ChartGrid {
  constructor(handler, gridConfig) {
    ...
    this._values = _lodash.default.defaultsDeep({}, gridConfig, defaults);
  }
  drawLine(svg, tick, axis, width, height) {
    ...
    svg.append('path').attr('d', () => {
      ...
      const d3Line = _d.default.svg.line().x(d => isHorizontal ? scale(d[0]) : d[0]).y(d => !isHorizontal ? scale(d[1]) : d[1]);
  ...
  draw(width, height) {
    ...
    return function (selection) {
      ...
      selection.each(function () {
        if (self.get('categoryLines')) self.drawCategoryLines(_d.default.select(this), width, height);
        if (self.get('valueAxis', false)) self.drawValueLines(_d.default.select(this), width, height);
      });
    };
  }
