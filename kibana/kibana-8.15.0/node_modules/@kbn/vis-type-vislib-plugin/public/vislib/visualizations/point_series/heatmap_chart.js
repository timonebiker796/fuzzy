 ...
 */
class HeatmapChart extends _point_series.PointSeries {
  constructor(handler, chartEl, chartData, seriesConfigArgs, uiSettings) {
    ...
    this.seriesConfig = _lodash.default.defaults(seriesConfigArgs || {}, defaults);
  ...
  getHeatmapLabels(cfg) {
    ...
    const percentageFormatPattern = cfg.get('percentageFormatPattern', this.uiSettings.get(_common.FORMATS_UI_SETTINGS.FORMAT_PERCENT_DEFAULT_PATTERN));
    ...
    if (cfg.get('setColorRange')) {
      ...
      for (let i = 0; i < colorsNumber; i++) {
        ...
        if (percentageMode) {
          val = (0, _percentage_mode_transform.getValueForPercentageMode)(val, percentageFormatPattern);
          nextVal = (0, _percentage_mode_transform.getValueForPercentageMode)(nextVal, percentageFormatPattern);
          label = `${val} - ${nextVal}`;
        } else {
          val = val * (max - min) + min;
  ...
  addSquares(svg, data) {
    ...
    if (this.getCategoryAxis().axisConfig.isTimeDomain()) {
      const {
        min,
        interval
      } = this.handler.data.get('ordered');
      const start = min;
      const end = (0, _moment.default)(min).add(interval).valueOf();
    ...
    function getColorBucket(d) {
      ...
      if (setColorRange && colorsRange.length) {
        const bucket = _lodash.default.find(colorsRange, range => {
    ...
    squares.append('rect').attr('x', x).attr('width', squareWidth).attr('y', y).attr('height', squareHeight).attr('data-label', label).attr('fill', z).attr('style', 'cursor: pointer; stroke: black; stroke-width: 0.1px').style('display', d => {
    ...
    if (showLabels) {
      ...
      const cellPadding = 5;
      const maxLength = Math.min(Math.abs(squareWidth / Math.cos(rotateRad)), Math.abs(squareHeight / Math.sin(rotateRad))) - cellPadding;
      const maxHeight = Math.min(Math.abs(squareWidth / Math.sin(rotateRad)), Math.abs(squareHeight / Math.cos(rotateRad))) - cellPadding;
      let labelColor;
      if (overwriteLabelColor) {
        ...
        labelColor = d => {
          ...
          return color && (0, _eui.isColorDark)(parseInt(color[1]), parseInt(color[2]), parseInt(color[3])) ? '#FFF' : '#222';
        };
      }
      squares.append('text').text(d => zAxisFormatter(d.y)).style('display', function (d) {
      ...
      }).style('dominant-baseline', 'central').style('text-anchor', 'middle').style('fill', labelColor).attr('x', function (d) {
