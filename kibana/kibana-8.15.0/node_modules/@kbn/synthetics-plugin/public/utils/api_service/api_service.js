  ...
  parseResponse(response, apiUrl, decodeType) {
    if (decodeType) {
      ...
      if ((0, _Either.isRight)(decoded)) {
        ...
        console.error(`API $s is not returning expected response, ${(0, _securitysolutionIoTsUtils.formatErrors)(decoded.left)} for response`, apiUrl, response);
      }
    }
    return response;
  }
  async get(apiUrl, params = {}, decodeType, asResponse = false, headers) {
