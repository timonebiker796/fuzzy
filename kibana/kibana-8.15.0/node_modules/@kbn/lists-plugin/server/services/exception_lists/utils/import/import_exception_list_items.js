...
const importExceptionListItems = async ({
  ...
  for await (const itemsChunk of itemsChunks) {
    ...
    const foundLists = await (0, _find_all_exception_list_types.getAllListTypes)(agnosticListItems.map(mapList), nonAgnosticListItems.map(mapList), savedObjectsClient);

    // Find any existing items with matching item_id
    // Dictionary of found items
    const foundItems = await (0, _find_all_exception_list_item_types.getAllListItemTypes)(agnosticListItems, nonAgnosticListItems, savedObjectsClient);
