    ...
    try {
      ...
      return response.ok({
        ...
        elasticsearchRole, roleName, authz.applicationName, logger)).filter(role => {
          var _role$metadata;
          return !(hideReservedRoles && (_role$metadata = role.metadata) !== null && _role$metadata !== void 0 && _role$metadata._reserved) && role.kibana.some(privilege => privilege.spaces.includes(request.params.spaceId) || privilege.spaces.includes(_constants.ALL_SPACES_ID));
