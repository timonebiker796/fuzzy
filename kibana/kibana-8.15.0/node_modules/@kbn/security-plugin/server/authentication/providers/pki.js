...
class PKIAuthenticationProvider extends _base.BaseAuthenticationProvider {
  ...
  async logout(request, state) {
    ...
    if (state) {
      try {
        ...
        this.logger.debug(`Failed invalidating access token: ${(0, _errors.getDetailedErrorMessage)(err)}`);
  ...
  async authenticateViaState(request, {
    ...
    if (!request.socket.authorized || peerCertificate === null || peerCertificate.fingerprint256 !== peerCertificateFingerprint256) {
      ...
      try {
        ...
        this.logger.error(`Failed to invalidate access token: ${(0, _errors.getDetailedErrorMessage)(err)}`);
  ...
  async authenticateViaPeerCertificate(request, state) {
    ...
    try {
      ...
      this.logger.debug(`Failed to exchange peer certificate chain to an access token: ${(0, _errors.getDetailedErrorMessage)(err)}`);
      return _authentication_result.AuthenticationResult.failed(err);
    }
