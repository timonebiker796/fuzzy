 ...
 */
let SAMLLogin = exports.SAMLLogin = /*#__PURE__*/function (SAMLLogin) {
...
class SAMLAuthenticationProvider extends _base.BaseAuthenticationProvider {
  ...
  async loginWithSAMLResponse(request, samlResponse, relayState, state) {
    ...
    try {
      ...
      this.logger.error(`Failed to log in with SAML response: ${(0, _errors.getDetailedErrorMessage)(err)}`);
    ...
    if (isIdPInitiatedLogin && relayState) {
      if (!this.useRelayStateDeepLink) {
        this.options.logger.warn(`"RelayState" is provided, but deep links support is not enabled for "${this.type}/${this.options.name}" provider.`);
      } else if (!(0, _std.isInternalURL)(relayState, this.options.basePath.serverBasePath)) {
        this.options.logger.warn(`"RelayState" is provided, but it is not a valid Kibana internal URL.`);
      } else {
  ...
  async authenticateViaRefreshToken(request, state) {
    ...
    try {
      ...
      this.logger.error(`Failed to refresh access token: ${(0, _errors.getDetailedErrorMessage)(err)}`);
      return _authentication_result.AuthenticationResult.failed(err);
    }
    ...
    if (refreshTokenResult === null) {
      if (canStartNewSession(request)) {
        this.logger.warn('Both access and refresh tokens are expired. Capturing redirect URL and re-initiating SAML handshake.');
