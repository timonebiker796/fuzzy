...
function disableUICapabilitiesFactory(request, features, elasticsearchFeatures, logger, authz, user) {
  ...
  const allRequiredIndexPrivileges = Object.values(elasticsearchFeatureMap).flat().filter(p => !!p.requiredIndexPrivileges).reduce((acc, p) => {
    return {
      ...
      ...Object.entries(p.requiredIndexPrivileges).reduce((acc2, [indexName, privileges]) => {
        var _acc$indexName;
        acc2[indexName] = [...((_acc$indexName = acc[indexName]) !== null && _acc$indexName !== void 0 ? _acc$indexName : []), ...privileges];
        return acc2;
      }, {})
    };
  }, {});
  const isCatalogueItemReferencedByFeatureSet = (catalogueEntry, featureSet) => {
    return featureSet.some(feature => {
      var _feature$catalogue;
      return ((_feature$catalogue = feature.catalogue) !== null && _feature$catalogue !== void 0 ? _feature$catalogue : []).includes(catalogueEntry);
    });
  };
  const shouldAffectCapability = (featureId, uiCapability) => {
    ...
    if (featureId === 'catalogue') {
      return uiCapability === 'spaces' || isCatalogueItemReferencedByFeatureSet(uiCapability, features) || isCatalogueItemReferencedByFeatureSet(uiCapability, elasticsearchFeatures);
    }
  ...
  const usingPrivileges = async uiCapabilities => {
    ...
    const uiActions = Object.entries(uiCapabilities).reduce((acc, [featureId, featureUICapabilities]) => {
      acc.push(...(0, _lodash.flatten)(Object.entries(featureUICapabilities).map(([uiCapability, value]) => {
    ...
    const checkPrivilegesForCapability = (enabled, featureId, ...uiCapabilityParts) => {
      ...
      if (!isElasticsearchFeature) {
        const hasRequiredKibanaPrivileges = checkPrivilegesResponse.privileges.kibana.some(x => x.privilege === action && x.authorized === true);
