...
async function getSplits(resp, panel, series, meta, extractFields) {
  ...
  const fieldsForSeries = (_meta = meta) !== null && _meta !== void 0 && _meta.dataViewId ? await extractFields({
    id: meta.dataViewId
  }) : [];
  ...
  series.metrics.filter(m => /_bucket/.test(m.type)).forEach(m => {
    (0, _.overwrite)(mergeObj, m.id, (0, _lodash.get)(resp, `aggregations.${series.id}.${m.id}`));
  });
  return [{
    id: series.id,
    splitByLabel,
