...
const {
  getGitRepoRootSync
} = require('./get_git_repo_root');
const {
  parse
} = require('../utils/jsonc');
const {
  isValidPluginCategoryInfo,
  PLUGIN_CATEGORY
} = require('./plugin_category_info');
...
const err = (key, value, msg) => {
  const dbg = ['string', 'number', 'boolean', 'undefined'].includes(typeof value) ? value : inspect(value);
...
function validatePackageManifestPlugin(plugin, repoRoot, path) {
  ...
  if (configPath !== undefined && !(isSomeString(configPath) || isArrOfStrings(configPath))) {
    throw err(`plugin.configPath`, configPath, `must be a non-empty string, or an array of non-empty strings`);
  }
  if (requiredPlugins !== undefined && !isArrOfIds(requiredPlugins)) {
    throw err(`plugin.requiredPlugins`, requiredPlugins, `must be an array of strings in camel or snake case`);
  }
  if (optionalPlugins !== undefined && !isArrOfIds(optionalPlugins)) {
    throw err(`plugin.requiredPlugins`, optionalPlugins, `must be an array of strings in camel or snake case`);
  }
  if (runtimePluginDependencies !== undefined && !isArrOfIds(runtimePluginDependencies)) {
    throw err(`plugin.runtimePluginDependencies`, runtimePluginDependencies, `must be an array of strings in camel or snake case`);
  }
  if (requiredBundles !== undefined && !isArrOfIds(requiredBundles)) {
    throw err(`plugin.requiredBundles`, requiredBundles, `must be an array of strings in camel or snake case`);
  }
  ...
  if (__category__ !== undefined) {
    if (!isBuild) {
      throw err('plugin.__category__', __category__, 'may only be specified on built packages in node_modules');
    }
    if (!isValidPluginCategoryInfo(__category__)) {
      throw err('plugin.__category__', __category__, 'is not valid');
    }
  } else if (isBuild) {
    throw err('plugin.__category__', __category__, 'must be defined on built packages in node_modules');
  }
  return {
    id,
    browser,
...
function validatePackageManifestBuild(build) {
  ...
  if (extraExcludes !== undefined && !isArrOfStrings(extraExcludes)) {
    throw err(`build.extraExcludes`, extraExcludes, 'must be an array of non-empty strings when defined');
  }
...
function validatePackageManifest(parsed, repoRoot, path) {
  ...
  if (!(Array.isArray(owner) && owner.every(isValidOwner)) && !(typeof owner === 'string' && isValidOwner(owner))) {
    throw err(`owner`, owner, `must be a valid Github team handle starting with @, or an array of such handles`);
  }
