...
const preResponseResult = {
  render(responseRender) {
    return {
      type: _coreHttpServer.OnPreResponseResultType.render,
      body: responseRender.body,
      headers: responseRender === null || responseRender === void 0 ? void 0 : responseRender.headers
    };
  },
  isRender(result) {
    return result && result.type === _coreHttpServer.OnPreResponseResultType.render;
  },
  next(responseExtensions) {
    return {
      type: _coreHttpServer.OnPreResponseResultType.next,
      headers: responseExtensions === null || responseExtensions === void 0 ? void 0 : responseExtensions.headers
    };
  },
  isNext(result) {
    return result && result.type === _coreHttpServer.OnPreResponseResultType.next;
  }
};
const toolkit = {
  ...
  return async function interceptPreResponse(request, responseToolkit) {
    ...
    try {
      if (response) {
        ...
        const result = await fn(_coreHttpRouterServerInternal.CoreKibanaRequest.from(request), {
          statusCode
        }, toolkit);
