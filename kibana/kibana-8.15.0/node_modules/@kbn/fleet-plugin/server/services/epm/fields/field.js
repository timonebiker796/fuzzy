...
exports.loadTransformFieldsFromYaml = exports.loadDatastreamsFieldsFromYaml = exports.isFields = exports.getField = exports.filterForTransformAssets = void 0;
...
const filterForTransformAssets = transformName => {
  return function isTransformAssets(path) {
    return path.includes(`/transform/${transformName}`);
  };
};
exports.filterForTransformAssets = filterForTransformAssets;
function combineFilter(...filters) {
  return function filterAsset(path) {
...
const loadDatastreamsFieldsFromYaml = (packageInstallContext, datasetName) => {
  // Fetch all field definition files
  const fieldDefinitionFiles = (0, _assets.getAssetsDataFromAssetsMap)(packageInstallContext.packageInfo, packageInstallContext.assetsMap, isFields, datasetName);
...
exports.loadDatastreamsFieldsFromYaml = loadDatastreamsFieldsFromYaml;
const loadTransformFieldsFromYaml = (packageInstallContext, transformName) => {
  // Fetch all field definition files
  const fieldDefinitionFiles = (0, _assets.getAssetsDataFromAssetsMap)(packageInstallContext.packageInfo, packageInstallContext.assetsMap, combineFilter(isFields, filterForTransformAssets(transformName)));
  return fieldDefinitionFiles.reduce((acc, file) => {
    // Make sure it is defined as it is optional. Should never happen.
    if (file.buffer) {
      const tmpFields = (0, _jsYaml.safeLoad)(file.buffer.toString());
...
exports.loadTransformFieldsFromYaml = loadTransformFieldsFromYaml;