...
const extractReferences = (workpad, expressions) => {
  ...
  const pages = workpad.pages.map(page => {
    const elements = page.elements.map(element => {
      const extract = expressions.extract((0, _interpreter.fromExpression)(element.expression));

      // Prefix references with the element id so we will know later which element it goes with
      references.push(...extract.references.map(reference => ({
        ...reference,
  ...
  const pages = workpad.pages.map(page => {
    const elements = page.elements.map(element => {
      const referencesForElement = references.filter(({
        name
      }) => name.indexOf(element.id) === 0).map(reference => ({
        ...reference,
