...
exports.ECS_MISSING_KEYS_PROMPT = exports.ECS_MAIN_PROMPT = exports.ECS_INVALID_PROMPT = exports.ECS_DUPLICATES_PROMPT = void 0;
...
const ECS_MAIN_PROMPT = exports.ECS_MAIN_PROMPT = _prompts.ChatPromptTemplate.fromMessages([['system', `You are a helpful, expert assistant in Elastic Common Schema (ECS), focusing only on helping users with translating their provided combined samples to Elastic Common Schema (ECS).
...
</context>`], ['human', `Looking at the combined sample from {package_name} {data_stream_name} provided above. The combined sample is a JSON object that includes all unique fields from the log samples sent by {package_name} {data_stream_name}.
...
{ex_answer}
\`\`\`
</example_response>"`], ['ai', 'Please find the JSON object below:']]);
const ECS_INVALID_PROMPT = exports.ECS_INVALID_PROMPT = _prompts.ChatPromptTemplate.fromMessages([['system', `You are a helpful, expert assistant in Elastic Common Schema (ECS), you help review and try to resolve incorrect field mappings. 
...
</context>`], ['human', `The following fields are mapped incorrectly in the current mapping, please help me resolve this:
<invalid_ecs_fields>
{invalid_ecs_fields}
</invalid_ecs_fields>
...
{ex_answer}
\`\`\`
</example>`], ['ai', 'Please find the JSON object below:']]);
const ECS_MISSING_KEYS_PROMPT = exports.ECS_MISSING_KEYS_PROMPT = _prompts.ChatPromptTemplate.fromMessages([['system', `You are a helpful, expert assistant in Elastic Common Schema (ECS), you help review and try to resolve missing fields in the current mapping.
...
</context>`], ['human', `The following keys are missing from the current mapping:
<missing_keys>
{missing_keys}
</missing_keys>
...
const ECS_DUPLICATES_PROMPT = exports.ECS_DUPLICATES_PROMPT = _prompts.ChatPromptTemplate.fromMessages([['system', `You are a helpful, expert assistant in Elastic Common Schema (ECS), you help review and try to resolve incorrect duplicate fields in the current mapping.
...
</context>`], ['human', `The following duplicate fields are mapped to the same ECS fields in the current mapping, please help me resolve this:
<duplicate_fields>
{duplicate_fields}
</duplicate_fields>
