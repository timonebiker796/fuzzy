...
exports.registerRelatedRoutes = registerRelatedRoutes;
...
var _related = require("../graphs/related");
...
function registerRelatedRoutes(router) {
  router.versioned.post({
    path: _common.RELATED_GRAPH_PATH,
    access: 'internal',
    options: {
      timeout: {
    ...
    validate: {
      request: {
        body: (0, _route_validation.buildRouteValidationWithZod)(_common.RelatedRequestBody)
      }
    }
    ...
    try {
      ...
      const graph = await (0, _related.getRelatedGraph)(client, model);
      const results = await graph.invoke(parameters, options);
      return res.ok({
        body: _common.RelatedResponse.parse(results)
      });
    } catch (e) {
