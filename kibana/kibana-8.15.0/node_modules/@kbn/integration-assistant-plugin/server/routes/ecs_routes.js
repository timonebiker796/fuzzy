...
exports.registerEcsRoutes = registerEcsRoutes;
...
function registerEcsRoutes(router) {
  router.versioned.post({
    ...
    validate: {
      request: {
        body: (0, _route_validation.buildRouteValidationWithZod)(_common.EcsMappingRequestBody)
      }
    }
    ...
    try {
      ...
      const graph = await (0, _ecs.getEcsGraph)(model);
      const results = await graph.invoke(parameters, options);
      return res.ok({
        body: _common.EcsMappingResponse.parse(results)
      });
    } catch (e) {
