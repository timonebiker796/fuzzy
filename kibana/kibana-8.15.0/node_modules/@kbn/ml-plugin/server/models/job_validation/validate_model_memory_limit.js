...
async function validateModelMemoryLimit(client, mlClient, job, duration) {
  ...
  const runCalcModelMemoryTest = duration && (duration === null || duration === void 0 ? void 0 : duration.start) !== undefined && (duration === null || duration === void 0 ? void 0 : duration.end) !== undefined;
  ...
  if (runCalcModelMemoryTest) {
    const {
      modelMemoryLimit
    } = await (0, _calculate_model_memory_limit.calculateModelMemoryLimitProvider)(client, mlClient)(job.analysis_config, job.datafeed_config.indices.join(','), job.datafeed_config.query, job.data_description.time_field, duration.start, duration.end, true, job.datafeed_config);
