...
function getSvgHeight(showAnnotations, incomingHeight) {
  ...
  const incomingHeightActual = incomingHeight && incomingHeight < minSvgHeight ? minSvgHeight : incomingHeight;
  const {
    focusHeight: focusHeightIncoming
  } = incomingHeight ? getChartHeights(incomingHeightActual) : {};
  return (focusHeightIncoming !== null && focusHeightIncoming !== void 0 ? focusHeightIncoming : focusHeight) + contextChartHeight + swimlaneHeight + adjustedAnnotationHeight + chartSpacing + margin.top + margin.bottom;
}
class TimeseriesChartIntl extends _react.Component {
  constructor(props) {
    super(props);
    (0, _defineProperty2.default)(this, "getTimeBuckets", void 0);
    ...
    (0, _defineProperty2.default)(this, "drawContextBrush", contextGroup => {
      ...
      function brushing() {
        ...
        const chartElement = _d.default.select(that.rootNode);
  ...
  componentDidMount() {
    ...
    this.getTimeBuckets = (0, _time_buckets_service.timeBucketsServiceFactory)(this.context.services.uiSettings).getTimeBuckets;
    const {
      svgWidth,
      svgHeight
    } = this.props;
    ...
    } = svgHeight ? getChartHeights(svgHeight) : {};
    ...
    this.focusYScale = _d.default.scale.linear().range([focusHeightIncoming !== null && focusHeightIncoming !== void 0 ? focusHeightIncoming : focusHeight, focusZoomPanelHeight]);
    const focusXScale = this.focusXScale;
    const focusYScale = this.focusYScale;
    this.focusXAxis = _d.default.svg.axis().scale(focusXScale).orient('bottom').innerTickSize(-(focusChartIncoming !== null && focusChartIncoming !== void 0 ? focusChartIncoming : focusChartHeight)).outerTickSize(0).tickPadding(10);
    ...
    this.rowMouseenterSubscriber = _table_service.mlTableService.rowMouseenter$.subscribe(tableRecordMousenterListener.bind(this));
  ...
  renderChart() {
    ...
    const {
      focusHeight: focusHeightIncoming
    } = incomingSvgHeight ? getChartHeights(incomingSvgHeight) : {};
    ...
    if (typeof selectedJob !== 'undefined') {
      this.fieldFormat = this.context.services.mlServices.mlFieldFormatService.getFieldFormat(selectedJob.job_id, detectorIndex);
    } else {
      return;
    }
    ...
    const context = svg.append('g').attr('class', 'context-chart').attr('transform', 'translate(' + margin.left + ',' + ((focusHeightIncoming !== null && focusHeightIncoming !== void 0 ? focusHeightIncoming : focusHeight) + margin.top + chartSpacing) + ')');
    ...
    annotationsMask.append('rect').attr('x', 0).attr('y', 0).attr('width', this.vizWidth).attr('height', focusHeightIncoming !== null && focusHeightIncoming !== void 0 ? focusHeightIncoming : focusHeight).style('fill', 'white');

    // Draw each of the component elements.
    createFocusChart(focus, this.vizWidth, focusHeightIncoming !== null && focusHeightIncoming !== void 0 ? focusHeightIncoming : focusHeight);
  ...
  createFocusChart(fcsGroup, fcsWidth, fcsHeight) {
    ...
    const {
      focusChartHeight: focusChartIncoming
    } = this.props.svgHeight ? getChartHeights(this.props.svgHeight) : {};
    ...
    fcsGroup.append('g').attr('class', 'mlAnnotationBrush').call(annotateBrush).selectAll('rect').attr('x', brushX).attr('y', focusZoomPanelHeight).attr('width', brushWidth).attr('height', focusChartIncoming !== null && focusChartIncoming !== void 0 ? focusChartIncoming : focusChartHeight);
    fcsGroup.append('g').classed('mlAnnotations', true);

    // Add border round plot area.
    fcsGroup.append('rect').attr('x', 0).attr('y', focusZoomPanelHeight).attr('width', fcsWidth).attr('height', focusChartIncoming !== null && focusChartIncoming !== void 0 ? focusChartIncoming : focusChartHeight).attr('class', 'chart-border');
  ...
  renderFocusChart() {
    ...
    (0, _timeseries_chart_annotations.renderAnnotations)(focusChart, focusAnnotationData, focusZoomPanelHeight, focusChartIncoming !== null && focusChartIncoming !== void 0 ? focusChartIncoming : focusChartHeight, this.focusXScale, showAnnotations, showFocusChartTooltip, hideFocusChartTooltip, this.props.annotationUpdatesService);

    // disable brushing (creation of annotations) when annotations aren't shown or when in embeddable mode
    focusChart.select('.mlAnnotationBrush').style('display', !showAnnotations || embeddableMode ? 'none' : null);
    ...
    const dots = chartElement.select('.focus-chart-markers').selectAll('.metric-value').data(data.filter(d => (d.value !== null || typeof d.anomalyScore === 'number') && !(0, _chart_utils.showMultiBucketAnomalyMarker)(d)));
    const that = this;
    ...
    }).attr('data-test-subj', d => d.anomalyScore !== undefined ? 'mlAnomalyMarker' : undefined).attr('class', d => {
    ...
    const multiBucketMarkers = chartElement.select('.focus-chart-markers').selectAll('.multi-bucket').data(data.filter(d => d.anomalyScore !== null && (0, _chart_utils.showMultiBucketAnomalyMarker)(d) === true));
    ...
    multiBucketMarkers.enter().append('path').attr('d', _d.default.svg.symbol().size(_chart_utils.MULTI_BUCKET_SYMBOL_SIZE).type('cross')).on('click', function (d) {
      _d.default.event.preventDefault();
      if (d.anomalyScore === undefined) return;
      showAnomalyPopover(d, this);
    }).on('mouseover', function (d) {
    ...
    multiBucketMarkers.attr('transform', d => `translate(${this.focusXScale(d.date)}, ${this.focusYScale(d.value)})`).attr('data-test-subj', 'mlAnomalyMarker').attr('class', d => `anomaly-marker multi-bucket ${(0, _mlAnomalyUtils.getSeverityWithLow)(d.anomalyScore).id}`);

    // Add rectangular markers for any scheduled events.
    const scheduledEventMarkers = chartElement.select('.focus-chart-markers').selectAll('.scheduled-event-marker').data(data.filter(d => d.scheduledEvents !== undefined));
    ...
    scheduledEventMarkers.attr('x', d => this.focusXScale(d.date) - _chart_utils.LINE_CHART_ANOMALY_RADIUS).attr('y', d => {
      ...
      return isNaN(focusYValue) ? -(focusHeightIncoming !== null && focusHeightIncoming !== void 0 ? focusHeightIncoming : focusHeight) - 3 : focusYValue - 3;
    });
    ...
    if (focusForecastData !== undefined) {
      ...
      const forecastDots = chartElement.select('.focus-chart-markers.forecast').selectAll('.metric-value').data(focusForecastData);
  ...
  highlightFocusChartAnomaly(record) {
    ...
    const markerToSelect = this.mlTimeSeriesExplorer.findChartPointForAnomalyTime(focusChartData, anomalyTime, focusAggregationInterval);
    const chartElement = _d.default.select(this.rootNode);
    ...
    if (markerToSelect !== undefined) {
      const selectedMarker = chartElement.select('.focus-chart-markers').selectAll('.focus-chart-highlighted-marker').data([markerToSelect]);
      if ((0, _chart_utils.showMultiBucketAnomalyMarker)(markerToSelect) === true) {
        selectedMarker.enter().append('path').attr('d', _d.default.svg.symbol().size(_chart_utils.MULTI_BUCKET_SYMBOL_SIZE).type('cross')).attr('transform', d => `translate(${focusXScale(d.date)}, ${focusYScale(d.value)})`).attr('data-test-subj', 'mlAnomalyMarker').attr('class', d => `anomaly-marker multi-bucket ${(0, _mlAnomalyUtils.getSeverityWithLow)(d.anomalyScore).id} highlighted`);
      } else {
        selectedMarker.enter().append('circle').attr('r', _chart_utils.LINE_CHART_ANOMALY_RADIUS).attr('cx', d => focusXScale(d.date)).attr('cy', d => focusYScale(d.value)).attr('data-test-subj', 'mlAnomalyMarker').attr('class', d => `anomaly-marker metric-value ${(0, _mlAnomalyUtils.getSeverityWithLow)(d.anomalyScore).id} highlighted`);
      }
...
(0, _defineProperty2.default)(TimeseriesChartIntl, "propTypes", {
  ...
  embeddableMode: _propTypes.default.bool,
...
(0, _defineProperty2.default)(TimeseriesChartIntl, "contextType", _public.context);
