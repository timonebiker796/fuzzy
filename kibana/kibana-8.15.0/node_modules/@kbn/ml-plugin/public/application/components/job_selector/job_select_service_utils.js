...
function getGroupsFromJobs(jobs) {
  ...
  Object.keys(groups).forEach(groupId => {
    ...
    group.timeRange.fromMoment = (0, _moment.default)(group.timeRange.from);
    // create label
...
function getTimeRangeFromSelection(jobs, selection) {
  if (jobs.length > 0) {
    ...
    if (times.length) {
      const extent = _d.default.extent(times);
      const selectedTime = {
...
function normalizeTimes(jobs, dateFormatTz, ganttBarWidth) {
  ...
  const ganttScale = _d.default.scale.linear().domain([min, max]).range([1, ganttBarWidth]);
  jobs.forEach(job => {
