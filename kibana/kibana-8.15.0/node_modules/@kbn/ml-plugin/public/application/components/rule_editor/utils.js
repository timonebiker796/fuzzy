...
function saveJobRule(job, detectorIndex, ruleIndex, editedRule, mlApiServices, mlJobService) {
  const detector = job.analysis_config.detectors[detectorIndex];
  const jobService = mlJobService || _job_service.mlJobService;
...
function deleteJobRule(job, detectorIndex, ruleIndex, mlApiServices, mlJobService) {
  const jobService = mlJobService || _job_service.mlJobService;
  const detector = job.analysis_config.detectors[detectorIndex];
  let customRules = [];
...
function updateJobRules(job, detectorIndex, rules, mlApiServices, mlJobService) {
  ...
  return new Promise((resolve, reject) => {
    mlApiServices.updateJob({
      jobId: jobId,
      job: jobData
    }).then(() => {
      // Refresh the job data in the job service before resolving.
      mlJobService.refreshJob(jobId).then(() => {
        resolve({
          success: true
        });
      }).catch(refreshResp => {
...
function addItemToFilter(item, filterId, mlApiServices) {
  return new Promise((resolve, reject) => {
    mlApiServices.filters.updateFilter(filterId, undefined, [item], undefined).then(updatedFilter => {
