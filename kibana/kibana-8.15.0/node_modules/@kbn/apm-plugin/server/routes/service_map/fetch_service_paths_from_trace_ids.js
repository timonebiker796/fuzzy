...
var _calculate_docs_per_shard = require("./calculate_docs_per_shard");
/*
...
const SCRIPTED_METRICS_FIELDS_TO_COPY = [_apm.PARENT_ID, _apm.SERVICE_NAME, _apm.SERVICE_ENVIRONMENT, _apm.SPAN_DESTINATION_SERVICE_RESOURCE, _apm.TRACE_ID, _apm.PROCESSOR_EVENT, _apm.SPAN_TYPE, _apm.SPAN_SUBTYPE, _apm.AGENT_NAME];
...
async function fetchServicePathsFromTraceIds({
  ...
  const serviceMapQueryDataResponse = await apmEventClient.search('get_trace_ids_shard_data', serviceMapParams);
  /*
  ...
  const calculatedDocs = (0, _calculate_docs_per_shard.calculateDocsPerShard)({
    serviceMapMaxAllowableBytes,
    avgDocSizeInBytes,
    totalShards,
    numOfRequests
  ...
  const serviceMapFromTraceIdsScriptResponse = await apmEventClient.search('get_service_paths_from_trace_ids', serviceMapParamsWithAggs);
