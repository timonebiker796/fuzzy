...
async function getNonHeapMemoryChart({
  ...
  const maxMemoryField = isOpenTelemetry ? _apm.METRIC_OTEL_JVM_PROCESS_MEMORY_LIMIT : _apm.METRIC_JAVA_NON_HEAP_MEMORY_MAX;
  const committedMemoryField = isOpenTelemetry ? _apm.METRIC_OTEL_JVM_PROCESS_MEMORY_COMMITTED : _apm.METRIC_JAVA_NON_HEAP_MEMORY_COMMITTED;
  const usedMemoryField = isOpenTelemetry ? _apm.METRIC_OTEL_JVM_PROCESS_MEMORY_USAGE : _apm.METRIC_JAVA_NON_HEAP_MEMORY_USED;
  const additionalFilters = isOpenTelemetry ? [{
    ...
    term: {
      [_apm.LABEL_TYPE]: _apm.VALUE_OTEL_JVM_PROCESS_MEMORY_NON_HEAP
    }
  }] : [{
    terms: {
      [_apm.AGENT_NAME]: _agent_name.JAVA_AGENT_NAMES
