...
exports.getMobileHttpRequests = getMobileHttpRequests;
...
async function getHttpRequestsTimeseries({
  ...
  const aggs = {
    requests: {
      filter: {
        term: {
          [_apm.SPAN_SUBTYPE]: _constants.MobileSpanSubtype.Http
        }
      }
    }
  };
  const response = await apmEventClient.search('get_http_requests_chart', {
    ...
    body: {
      ...
      query: {
        bool: {
          filter: [...(0, _server.termQuery)(_apm.SPAN_TYPE, _constants.MobileSpanType.External), ...(0, _server.termQuery)(_apm.SERVICE_NAME, serviceName), ...(0, _server.termQuery)(_apm.SPAN_TYPE, 'external'), ...(0, _server.termQuery)(_apm.TRANSACTION_NAME, transactionName), ...(0, _server.rangeQuery)(startWithOffset, endWithOffset), ...(0, _environment_query.environmentQuery)(environment), ...(0, _server.kqlQuery)(kuery)]
        }
      },
      aggs: {
        timeseries: {
  ...
  return {
    timeseries,
    value: (_response$aggregation3 = response.aggregations) === null || _response$aggregation3 === void 0 ? void 0 : (_response$aggregation4 = _response$aggregation3.requests) === null || _response$aggregation4 === void 0 ? void 0 : _response$aggregation4.doc_count
  };
}
async function getMobileHttpRequests({
  kuery,
  apmEventClient,
  serviceName,
  transactionName,
  ...
  const previousPeriodPromise = offset ? getHttpRequestsTimeseries({
    ...options,
    start,
    end,
    offset
